...
from swiss_energy_mcp.server import CORS_ROUTING_HEADERS, build_http_app, mcp
...
def client(monkeypatch: pytest.MonkeyPatch) -> TestClient:
    _ = monkeypatch
    # Settings direkt gebaut statt ueber die Umgebung: `cors_origins` ist ein
    # `list[str]`, und pydantic-settings JSON-dekodiert komplexe Felder in der
    # Env-Quelle, BEVOR der `mode="before"`-Validator laeuft. Ein
    # kommaseparierter Wert wirft dort `SettingsError` — siehe den Hinweis im
    # PR-Text. Der Test soll CORS pruefen, nicht diese Falle erben.
    return TestClient(build_http_app(Settings(cors_origins=[ORIGIN])))
